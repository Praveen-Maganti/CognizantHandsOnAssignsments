...
CREATE OR REPLACE PACKAGE EmployeeManagement AS
    ...
    PROCEDURE UpdateEmployeeDetails(p_id IN NUMBER, p_pos IN VARCHAR2, p_salary IN NUMBER);
    FUNCTION CalculateAnnualSalary(p_id IN NUMBER) RETURN NUMBER;
END EmployeeManagement;
/

CREATE OR REPLACE PACKAGE BODY EmployeeManagement AS
    PROCEDURE HireEmployee(p_id IN NUMBER, p_name IN VARCHAR2, p_pos IN VARCHAR2, p_salary IN NUMBER, p_dept IN VARCHAR2) IS
    BEGIN
    ...
    PROCEDURE UpdateEmployeeDetails(p_id IN NUMBER, p_pos IN VARCHAR2, p_salary IN NUMBER) IS
    BEGIN
        UPDATE Employees
        SET Position = p_pos, Salary = p_salary
...
CREATE OR REPLACE PACKAGE AccountOperations AS
    PROCEDURE OpenAccount(p_id IN NUMBER, p_cust_id IN NUMBER, p_type IN VARCHAR2, p_balance IN NUMBER);
...
CREATE OR REPLACE PACKAGE BODY AccountOperations AS
    PROCEDURE OpenAccount(p_id IN NUMBER, p_cust_id IN NUMBER, p_type IN VARCHAR2, p_balance IN NUMBER) IS
    BEGIN
    ...
    PROCEDURE CloseAccount(p_id IN NUMBER) IS
    BEGIN
